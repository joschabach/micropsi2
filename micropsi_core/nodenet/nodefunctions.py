...
def script(nodenet, node=None, **params):
    """ Script nodes are state machines that use the node activation for determining their behavior.
        ...
        The links have the following semantics:
            ...
            - por determines successors. If an action is neither inactive nor confirmed, it will inhibit its successors
              ...
              action in a por-chain receives no such inhibition, it may turn from "ready" to "requesting". After it
            ...
            - ret determines predecessors. Successors inhibit their predecessors from telling their parents when they
              confirm. Once an action without sucessor confirms, we assume that the sub-actions have successfully
              realized the parent action.
        ...
        < 0.01  inactive (will change to ready when requested)
        < 0.3   waiting (will change to ready when no longer inhibited)
        < 0.5   ready (inhibits neighbors and goes to waiting)
    ...
    node.activation = (
        -1. if node.activation < 0 else  # failed -> failed
        0.4 if node.activation < 0.01 else  # (we already tested that sub is positive): inactive -> ready
        0.2 if node.activation < 0.5 and node.get_slot("por").activation < 0 else  # waiting/ready -> waiting
        node.activation + 0.2 if node.activation < 0.7 else  # waiting -> ready -> requesting -> pending
        1.0 if node.get_slot("sur").activation >= 1 else  # pending -> confirmed
        -1. if node.get_slot("sur").activation <= 0 else  # pending -> failed
        node.activation
    )
    """
    if   node.activation < 0: node.activation = -1.
    elif node.activation < 0.01: node.activation = 0.4
    elif node.activation < 0.5 and node.get_slot("por").activation <0: node.activation = 0.2
    elif node.activation < 0.7: node.activation += 0.2
    elif node.get_slot("sur").activation >= 1: node.activation = 1.
    elif node.get_slot("sur").activation <= 0: node.activation = -1.
    """

    # always inhibit successor, except when confirmed
    node.get_gate("por").gate_function(-1 if node.activation<1 else 1)
    # always inhibit confirmation of predecessor
    node.get_gate("ret").gate_function(-1)
    # request children when becoming requesting
    node.get_gate("sub").gate_function(0 if node.activation<0.5 or node.activation >=100 else 1)
    # keep parent from failing while waiting, pending or processing, confirm parent when confirmed
    node.get_gate("sur").gate_function(0 if node.activation<0.5 else
                                       0.01 if node.activation<1 or node.get_slot("ret").activation>0 else
                                       1)
