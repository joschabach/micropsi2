...
class TheanoNodenet(Nodenet):
    ...
    def worldadapter_instance(self, _worldadapter_instance):
        ...
        self.regenerate_worldadapter_flowtypes()
    ...
    def regenerate_worldadapter_flowtypes(self):
        """ (Re)Generates flow-modules (and their types) for datasources and targets from the worldadapter"""
        ...
        for uid in list(self.flow_module_instances.keys()):
            if self.flow_module_instances[uid].type == key:
                self.delete_node(uid)

        nodes = []
        for out_group in self.worldadapter_instance.datasource_groups:
            key = 'wa_out_' + out_group
            self.native_module_definitions[key] = {
                'flow_module': True,
            ...
            self.native_modules[key] = FlowNodetype(nodenet=self, **self.native_module_definitions[key])
            nodes.append((key, out_group))
        for in_group in self.worldadapter_instance.datatarget_groups:
            key = 'wa_in_' + in_group
            self.native_module_definitions[key] = {
                'flow_module': True,
                'implementation': 'python',
            ...
            self.native_modules[key] = FlowNodetype(nodenet=self, **self.native_module_definitions[key])
            nodes.append((key, in_group))
        for key, name in nodes:
            self.create_node(key, None, [100, 100], name=name)
