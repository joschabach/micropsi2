...
def test_activation_recorder(test_nodenet, resourcepath):
...
@pytest.mark.engine("theano_engine")
def test_linkweight_recorder(test_nodenet, resourcepath):
    nodenet = micropsi.nodenets[test_nodenet]
    netapi = nodenet.netapi
    nodespace = netapi.get_nodespace(None)
    layer1 = []
...
@pytest.mark.engine("theano_engine")
def test_clear_recorder(test_nodenet, resourcepath):
    nodenet = micropsi.nodenets[test_nodenet]
    netapi = nodenet.netapi
    nodespace = netapi.get_nodespace(None)
    for i in range(5):
...
@pytest.mark.engine("theano_engine")
def test_remove_recorder(test_nodenet, resourcepath):
    nodenet = micropsi.nodenets[test_nodenet]
    netapi = nodenet.netapi
    nodespace = netapi.get_nodespace(None)
    for i in range(5):
