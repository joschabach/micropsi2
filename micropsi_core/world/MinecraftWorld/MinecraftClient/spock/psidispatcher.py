...
class PsiDispatcher():
    ...
    def dispatchPsiCommands(self):
        ...
        if self.client.move_x > 0:
            current_block = current_section['block_data'].get(int((self.client.position['x'] - 1)  // 1 % 16),
                                                              int(self.client.position['y'] // 1 % 16),
                                                              int(self.client.position['z'] // 1 % 16))
            if current_block == 0:
        ...
        if self.client.move_z > 0:
            current_block = current_section['block_data'].get(int(self.client.position['x']  // 1 % 16),
                                                              int(self.client.position['y'] // 1 % 16),
                                                              int((self.client.position['z'] - 1) // 1 % 16))
            if current_block == 0:
                self.client.push(Packet(ident = 0x0B, data = {
