...
class PsiDispatcher():
    ...
    def dispatchPsiCommands(self):
        ...
        if move_x < 0 and move_z < 0:
            target_coords = (self.normalize_coordinate(x + STEP_LENGTH),
                              y,
                              self.normalize_coordinate(z + STEP_LENGTH))
            self.move(target_coords, current_section, "x+z+")
        elif move_x > 0 and move_z < 0:
            target_coords = (self.normalize_coordinate(x + STEP_LENGTH),
                              y,
                              self.normalize_coordinate(z - STEP_LENGTH))
            self.move(target_coords, current_section, "x-z+")
            ...
            target_coords = (self.normalize_coordinate(x),
                              y,
                              self.normalize_coordinate(z + STEP_LENGTH))
